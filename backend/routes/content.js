...
router.get('/', async (req, res) =>{
  try{
    const {search, difficulty, page=1, limit=10, sortBy = 'title_asc'} = req.query;

    let questionQuery = {};
    
    if(search) questionQuery.title = {$regex: search, $options: 'i'};
    
    if(difficulty && ['Easy','Medium','Hard'].includes(difficulty)) questionQuery.difficulty = difficulty;
    ...
    let categories = await Category.find().populate({path: 'questions', match: questionQuery, select: 'title url difficulty'});
    ...
    categories.forEach((cat) =>{
      cat.questions.sort((a, b) =>{
        if(sortField === 'difficulty'){
          return sortDirection*(difficultyOrder[a.difficulty]-difficultyOrder[b.difficulty]);
        }
...
router.get('/questions', async (req, res)=>{
  try{
    ...
    if(sortField === 'difficulty'){
      const difficultyOrder = {'Easy': 1, 'Medium': 2, 'Hard': 3};
      questions.sort((a, b) => sortDirection*(difficultyOrder[a.difficulty]-difficultyOrder[b.difficulty]));
    } 
    else{
      questions.sort((a, b) => sortDirection*String(a[sortField]||'').localeCompare(String(b[sortField]||'')));
    }
