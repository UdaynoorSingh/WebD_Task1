...
const seedDatabase = async ()=>{
  try{
    ...
    if(!Array.isArray(sourceCategories)) throw new Error('Fetched data is not in the expected format.');
    

    const allQuestionDocs = sourceCategories
      .flatMap(category => category.ques || [])
      .filter(item => item.title && item.p1_link)
      .map(item =>({
        title: item.title,
        url: item.p1_link
      }));
    ...
    const categoryDocs = sourceCategories
      .map(category =>{
        const questionIds = (category.ques || [])
          .map(item =>questionMap.get(item.p1_link))
          .filter(id =>id);
